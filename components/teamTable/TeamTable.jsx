...
const TeamTable = () => {
  ...
  useEffect(() => {
    ...
    get(dbRef)
      .then((snapshot) => {
        if (snapshot.exists()) {
          const mappedTeams = Object.entries(snapshot.val())
            .filter(([key, value]) => value.owner == auth.currentUser.uid)
            .map(([key, value]) => ({
              id: key,
              ...value,
            }));
      ...
      <div className="flex justify-center h-fit pb-10  w-full">
        {isLoading ? (
          ...
          <div className="flex flex-col h-[75vh] justify-center items-center">
            ...
            <h1 className="text-center font-bold text-3xl font-sourceSans">
              It seems that there is no team yet
            </h1>
            <p className=" font-bold font-sourceSans text-xl">
              go create one now, just click me!
            </p>
          </div>
        )}
      </div>
